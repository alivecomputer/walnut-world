...
import { Apple, Monitor, Send, ChevronDown } from 'lucide-react'
...
export function Hero({ onDownloadClick }: HeroProps) {
  ...
  return (
    <section
      className="relative"
      style={{ height: isMobile ? '250vh' : '100vh' }}
    >
      {/* Sticky inner — stays centered while scroll drives the video */}
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center px-4 text-center">
        ...
        {isMobile && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
            <ChevronDown className="size-5 animate-bounce text-cream/20" />
          </div>
        )}
      </div>
    </section>
